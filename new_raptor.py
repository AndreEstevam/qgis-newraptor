...
class NewRaptor:
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.cmbSpecies.currentTextChanged.connect(self.evt_cmbSpecies_changed)     # automatic buffer
            
        # automatic latitude, longitude and date
        ...
        # checking if needed layers are loaded
        map_layers = []
        ...
        if result:
            ...
            # Setting it's attributes and geometry
            ...
            # To feed a layer we need a Data Provider. QGis can receive data in many ways and forms (.shp, .json, .geopackage);
            # to handle theses, QGis must know the source of it, and that the Data Provider knows.
            # Imagine it like a dish: each dish (soup, bread, meat) requires a different tool (spoon, hands, fork)
            # to be ingested;
            ...
            geom_buff = geom.buffer(valBuffer, 10)
            ftrNest.setGeometry(geom_buff)
            pr.addFeatures([ftrNest])
            lyrBuffer.reload()
            ...
            QMessageBox.information(self.dlg, "Message", "New nest ID: {}\nSpecies: {}\nLatitude: {}".format(valNestID, valSpecies, valLat))
        else:
            QMessageBox.information(self.dlg, "Message", "Should run only when cancelled")
